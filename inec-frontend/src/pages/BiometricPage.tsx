...
export default function BiometricPage() {
  ...
  useEffect(() => {
    ...
    api.getAdvancedBiometricStats().then(setAdvStats).catch(() => {});
    api.getHSMStats().then(setHsmStats).catch(() => {});
    api.getSDKProviders().then(setSdkProviders).catch(() => {});
    api.getTemplateAging().then(setAgingData).catch(() => {});
    api.getCancelableStatus().then(setCancelableData).catch(() => {});
    api.getThresholdTuning().then(setTuningData).catch(() => {});
    api.getPADModels().then(setPadModels).catch(() => {});
    api.getQualityGateway().then(setQualityGW).catch(() => {});
    api.getOfflineQueue().then(setOfflineQueue).catch(() => {});
    api.getScoreCohorts().then(setScoreCohorts).catch(() => {});
    api.getNISTBenchmarks().then(setNistBench).catch(() => {});
    api.getBioAuditTimeline(50).then(setAuditTimeline).catch(() => {});
    api.getBioAuditSummary().then(setAuditSummary).catch(() => {});
    api.getKioskSessions().then(setKioskSessions).catch(() => {});
    api.getMultiFingerStatus().then(setMultiFingerData).catch(() => {});
    api.getPrivacyStats().then(setPrivacyStats).catch(() => {});
  }, []);
