...
// initMiddlewareTables creates PostgreSQL-backed tables for middleware state persistence.
func initMiddlewareTables(database *sql.DB) {
	tables := []string{
		// Redis-like key-value cache with TTL
		...
		// TigerBeetle-like ledger accounts
		...
		// TigerBeetle-like ledger transfers
		...
		// Pub/Sub subscriptions registry
	...
	for _, ddl := range tables {
		if _, err := database.Exec(ddl); err != nil {
			log.Printf("WARNING: failed to create middleware table: %v", err)
		}
	}
	for _, idx := range indexes {
		if _, err := database.Exec(idx); err != nil {
			log.Printf("WARNING: failed to create middleware index: %v", err)
		}
	}
