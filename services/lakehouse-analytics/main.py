...
class AnomalyDetector:
    ...
    def __init__(self):
        ...
        self._fit_threshold = 0.1  # Refit if data size changes by >10%

    def _should_refit(self, data_size: int) -> bool:
        """Only refit if model is unfitted or data has changed significantly."""
        if not self.is_fitted:
            return True
        if self._last_fit_size == 0:
            return True
        change_ratio = abs(data_size - self._last_fit_size) / self._last_fit_size
        return change_ratio > self._fit_threshold
    ...
    def detect_anomalies(self, vote_counts: list[int]) -> list[AnomalyResult]:
        ...
        # Only refit if data has changed significantly (avoids O(n²) on repeated calls)
        if self._should_refit(len(vote_counts)):
            self.model.fit(arr)
            self.is_fitted = True
            self._last_fit_size = len(vote_counts)
