...
After the first independent re-score (Go 78, Rust 70, Python 73, Infra 72, Campaign 74, Frontends 60 — aggregate 71), a second fix wave closed every docked item, followed by a mop-up wave for the second re-score's residuals:

**R95 wave (6 branches, merged @ `a93db9e`):**
- **Go:** all WS/SSE surfaces (`/results/ws/updates`, sharded, `/observer/stream`, `/dashboard/stream`, `/gotv/ws`) authenticate via `authenticateStreamRequest` — Bearer or HttpOnly `inec_token` cookie, `type=="access"` + jti blacklist; `?token=` rejected outright in production. Dev-login endpoints 404 outside dev mode. `GOTV_DEV_MODE=true` in production → startup `log.Fatal` (double guard). Wildcard CORS headers deleted from 9 handler sites. `validateConfig` enforces every REQUIRED-IN-PROD var. `/metrics` unguarded in prod → startup fatal. New backend README. **93 lines of security theater deleted** (unwired `jwtValidationMiddleware` + helpers) after proving all ~198 gotv-svc routes are protected per-route. 20+ new auth tests.
- **Rust:** Dockerfiles for rust-hot-path + geolibre-spatial (non-root, `--locked`, wired into CI + compose). Constant-time key comparison in all 5 key-checked services (zero `v == expected_key` remain). gotv-engine `/health` → HTTP 503 when persistence degraded. Tenancy fail-closed: explicit `GOTV_SINGLE_TENANT_MODE=true` opt-in, prod startup exit, dapr-only calls → 403 `party_binding_required`. Comma-separated zero-downtime key rotation on all 7 key envs. READMEs for all 7 services. inference-engine v1 in CI matrix; pre-existing wrong haversine test fixed.
- **Python:** campaign-planning / federated / digital-twin in-memory state → Postgres (write-through + startup reload, prod fail-fast). Tally durable under compose (`APP_ENV`+`DATABASE_URL` wired). Prod fail-fast on missing secrets in all 14 FastAPI services. biometric `/health` → 6 real probes with 503 degradation. Docs gated in prod in all 14 services. httpx timeout budgets + bounded retries. 24 new TestClient tests. hmac.compare_digest + key rotation in 13 services.
- **Infra:** `:?`-guard ↔ `.env.example` set-difference **EMPTY** (NEO4J_PASSWORD + 8 more added). Resource limits + restart policies on **all** compose services (9 files). biometric-rust non-root. curl healthchecks on curl-less images replaced. campaign-platform CI job (pnpm, tsc, vitest) + node 20→22 alignment.
- **Campaign:** **CRITICAL — `requireProfileRole` was live-broken**: tRPC v11 leaves middleware `input` undefined until a parser runs, so every profileScopedProcedure threw BAD_REQUEST on every call; the Round-2 tenancy layer never functioned over HTTP. Fixed via `getRawInput()` + regression tests (premise independently proven by the verifier against the installed tRPC). Postgres-backed `rate_limits` table (atomic upsert) for login throttle / LLM limit / petition dedup — prod fails closed, multi-instance safe. `/metrics` Prometheus endpoint (guarded, fail-closed) + structured JSON logs. 5 new test files (24 tests). petitions.sign requires manager. `_old-migrations-backup/` deleted.
- **Frontends:** mobile SSE JWT out of URL (header-based SSE reader). MapPage.tsx 5 `setHTML` XSS sinks → `setDOMContent` + `textContent` via new `popup-dom.ts` (+3 sanitizer tests). Hardcoded `spatialStats(1)` / `electionId || 1` eliminated — electionId required everywhere, geo-map gated on resolver. JWT moved to in-memory store (localStorage scrubbed in prod). nginx CSP + frame/nosniff/referrer/permissions headers. Duplicate pnpm lockfile deleted.

**Mop-up wave (3 branches, merged @ `1f20102`):**
- **Go:** gotv-svc CORS **failed open with credentials** when `GOTV_CORS_ORIGINS` unset — now deny-default (prod fatal / dev credential-less wildcard / listed-origins-only). GOTV_URL/GOTV_ENGINE_URL/GOTV_ANALYTICS_URL enforced in prod. Production detection unified (`APP_ENV` **or** `INEC_ENV`) across 22 files.
- **Python:** smoke tests extended to all remaining FastAPI services (federated, tally, ai-anomaly, biometric, gotv-analytics, lakehouse, sedona, pipeline-optimizer). New tests caught a real bug: pipeline-optimizer `/api/v1/ingest` returned HTTP 200 for a 503 condition (Flask-style tuple) — fixed.
- **Infra:** healthchecks for every first-party service (go-backend, frontend, caddy, geolibre-*, federated, digital-twin, sedona, redis, openappsec-db) + justification comments for 14 third-party/distroless images — census **42/42** services have restart + limits + (healthcheck OR documented justification). fluvio-stream + inference-engine(-v2) added to CI docker matrix.

## 9. Applying the fixes (final, supersedes §7)

```bash
git clone https://github.com/munisp/inec && cd inec
git checkout 1d80c54
git apply /path/to/inec-mockware-fixes.patch   # cumulative: rounds 1–3 + R95 + mop-up

# The patch deliberately excludes 4 binary blobs (git apply cannot encode binary
# deletions without a 268 MB payload). Remove them yourself — they are junk/PII:
git rm inec-go-backend/go1.22.4.linux-amd64.tar.gz \
       inec-go-backend/go1.22.4.linux-amd64.tar.gz.1 \
       inec-go-backend/go1.22.4.linux-amd64.tar.gz.2 \
       tourismpay-backup-20260722-2146.sql.gz
# NOTE: the tourismpay PII dump remains in git HISTORY after this — purging
# history (git filter-repo + force push) is the repo owner's decision.
```

Verified: `git apply --check` passes on pristine `1d80c54` (final stats in `PRODUCTION_READINESS_REPORT.md`). The in-repo copies of this report and `PRODUCTION_READINESS_REPORT.md` live at the repository root in the delivery commit `6630f82`-onward, satisfying the references from `README.md`, `AI_ML_PRODUCTION_AUDIT.md`, and `AUDIT_REPORT.md`.
